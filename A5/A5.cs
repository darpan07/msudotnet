internal interface IVehicle {
    ...
    internal class BMW : IVehicle {
        ...
        internal BMW(string type, int seater, string number,IVehicle.Renttype rentType, int age, int rentperunit, DateOnly last_maintenance_date) {
            this.type = type;
            this.seater = seater;
            this.number = number;
            renttype = rentType;
    ...
    internal class tata : IVehicle {
        internal string? type, number;
        internal IVehicle.Renttype renttype;
        internal int age, rentperunit, seater;
        internal DateOnly last_maintenance_date;

        internal tata(string type, int seater, string number,IVehicle.Renttype rentType, int age, int rentperunit, DateOnly last_maintenance_date) {
            this.type = type;
            this.seater = seater;
            this.number = number;
            renttype = rentType;
    ...
    internal class harleydavidson : IVehicle {
        internal string? type, number;
        internal IVehicle.Renttype renttype;
        internal int age, rentperunit, seater;
        internal DateOnly last_maintenance_date;

        internal harleydavidson(string type, int seater, string number,IVehicle.Renttype rentType, int age, int rentperunit, DateOnly last_maintenance_date) {
            this.type = type;
            this.seater = seater;
            this.number = number;
            renttype = rentType;
        ...
        public void getDetails() 
        {
            Console.Write("Brand : harleydavidson \n");
            Console.Write($"Car Number : {number}\n");
            Console.Write($"Total Seats : {seater}\n");
            Console.Write($"Type : {type}\n");
            Console.Write($"Car age : {age}\n");
...
class Program {
    static void Main(string[] args) {
        ...
        BMW i1 = new BMW("Petrol", 5, "GJ-06-KN-0024", IVehicle.Renttype.Day, 10, 13, new DateOnly(2020, 12,16 ));

        MercedesBenz mb1 = new MercedesBenz("Diesel",7, "GJ-01-AB-7875", IVehicle.Renttype.KM, 3, 17, new DateOnly(2021, 07,18 ));

        tata q1 = new tata("Diesel", 7, "UP-04-GS-5700", IVehicle.Renttype.KM, 7, 5, new DateOnly(2021, 11,21));

        tata q2 = new tata("CNG", 4, "MH-01-HI-0210", IVehicle.Renttype.KM, 15, 14, new DateOnly(2022, 02,28 ));
